...
export function FirstLoginPage({ token, onDone }: { token?: string; onDone: () => void }) {
  ...
  const [error, setError] = useState(token ? "" : "Invitation invalide ou expiree.");
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!validatePasswordRules(password).valid || password !== confirm) {
      setError(password !== confirm ? "Les deux mots de passe ne correspondent pas." : "Le mot de passe ne respecte pas les regles minimales.");
      return;
    }
    setLoading(true);
    setError("");
    try {
      ...
      setMessage("Votre compte est active. Vous pouvez vous connecter.");
    } catch {
      setError("Invitation invalide, expiree ou deja utilisee.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthLayout title="Activation de votre compte" description="Definissez votre mot de passe pour finaliser la premiere connexion.">
      <form className="space-y-4" onSubmit={submit}>
        ...
        <button type="button" className="w-full rounded-md border border-line px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-surface" onClick={onDone}>Retour a la connexion</button>
      </form>
    </AuthLayout>
  );
}
