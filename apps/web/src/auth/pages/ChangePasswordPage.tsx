...
export function ChangePasswordPage({ onDone }: { onDone: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setError("Changement impossible. Vérifiez le mot de passe actuel et réessayez.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthLayout title="Changer mon mot de passe" description="Mettez à jour votre mot de passe de connexion.">
      <form className="space-y-4" onSubmit={submit}>
        ...
        <button type="button" className="w-full rounded-md border border-line px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-surface" onClick={onDone}>Retour à l'application</button>
      </form>
    </AuthLayout>
  );
}
