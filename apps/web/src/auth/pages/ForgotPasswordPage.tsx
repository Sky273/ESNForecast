...
export function ForgotPasswordPage({ onBack }: { onBack: () => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      setMessage("Si un compte existe avec cette adresse, un email de reinitialisation a ete envoye.");
    } catch {
      setMessage("Si un compte existe avec cette adresse, un email de reinitialisation a ete envoye.");
    } finally {
      setLoading(false);
  ...
  return (
    <AuthLayout title="Mot de passe oublie" description="Demandez un lien de reinitialisation de mot de passe.">
