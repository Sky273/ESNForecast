...
export function ConnectedFinanceDashboard({ scenarioId, horizon }: V3Context) {
  ...
  return (
    <section className="space-y-5">
      <PageTitle title="Dashboard V3 finance connectee" subtitle="Tresorerie bancaire, ecarts, rapprochement, fiabilite, runway et qualite des donnees." />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <ChartCard title="Tresorerie prevue / reelle / recalibree">
          <LineChart data={data?.treasury ?? []}>
            ...
            <Legend />
            <Line dataKey="forecastClosingCash" name="Prevu" stroke="#64748b" strokeWidth={2} />
            <Line dataKey="actualClosingCash" name="Reel bancaire" stroke="#0f766e" strokeWidth={2} />
            <Line dataKey="recalibratedClosingCash" name="Recalibre" stroke="#2563eb" strokeWidth={2} />
          </LineChart>
        </ChartCard>
        <ChartCard title="Fiabilite previsionnelle">
      ...
      <SimpleTable rows={data?.dataQualityIssues ?? []} columns={[
        ["severity", "Severite", severityBadge],
        ["type", "Type"],
        ["message", "Message"],
...
export function BankAccountsPage() {
  return <CrudPage title="Banque - comptes" path="/bank/accounts" initial={{ organizationId: "", companyId: "", bankConnectionId: "", externalAccountId: "", name: "", ibanMasked: "FR76********1234", currency: "EUR", type: "checking", currentBalance: 0, availableBalance: 0, balanceDate: "2026-06-30", isActive: true }} fields={[
    { name: "organizationId", label: "Organisation ID" }, { name: "companyId", label: "Societe ID" }, { name: "bankConnectionId", label: "Connexion ID" }, { name: "externalAccountId", label: "Compte externe" }, { name: "name", label: "Nom" }, { name: "ibanMasked", label: "IBAN masque" }, { name: "currentBalance", label: "Solde", type: "number" }, { name: "availableBalance", label: "Disponible", type: "number" }, { name: "balanceDate", label: "Date solde", type: "date" }, { name: "isActive", label: "Actif", type: "checkbox" }
  ]} columns={[{ key: "name", label: "Compte" }, { key: "ibanMasked", label: "IBAN masque" }, { key: "currentBalance", label: "Solde", render: (row: any) => money(row.currentBalance) }, { key: "availableBalance", label: "Disponible", render: (row: any) => money(row.availableBalance) }, { key: "balanceDate", label: "Date" }]} />;
}
...
export function BankTransactionsPage() {
  return <CrudPage title="Transactions bancaires" path="/bank/transactions" initial={{ organizationId: "", companyId: "", bankAccountId: "", externalTransactionId: "", transactionDate: "2026-06-30", bookingDate: "2026-06-30", label: "", amount: 0, currency: "EUR", direction: "debit", status: "booked", categorizationStatus: "uncategorized", reconciliationStatus: "unreconciled", confidenceScore: 0 }} fields={[
    { name: "organizationId", label: "Organisation ID" }, { name: "companyId", label: "Societe ID" }, { name: "bankAccountId", label: "Compte ID" }, { name: "externalTransactionId", label: "ID externe" }, { name: "transactionDate", label: "Date", type: "date" }, { name: "label", label: "Libelle" }, { name: "counterpartyName", label: "Contrepartie" }, { name: "amount", label: "Montant", type: "number" }, { name: "direction", label: "Sens" }, { name: "categoryId", label: "Categorie ID" }, { name: "categorizationStatus", label: "Categorisation" }, { name: "reconciliationStatus", label: "Rapprochement" }
  ]} columns={[{ key: "transactionDate", label: "Date" }, { key: "label", label: "Libelle" }, { key: "counterpartyName", label: "Contrepartie" }, { key: "amount", label: "Montant", render: (row: any) => money(row.amount) }, { key: "categorizationStatus", label: "Categorie" }, { key: "reconciliationStatus", label: "Rapprochement" }]} />;
}

export function BankReconciliationPage() {
  const { rows } = useRows("/reconciliation/suggestions");
  return <TablePage title="Rapprochement bancaire" subtitle="Suggestions entre transactions, factures, paiements, couts et taxes." rows={rows} columns={[
...
export function ImportedAccountingPage() {
  ...
  return <TablePage title="Comptabilite importee" subtitle="Factures et paiements importes depuis connecteur mock ou CSV." rows={rows} columns={[
...
export function RealTreasuryPage({ scenarioId, horizon }: V3Context) {
  ...
  return <TablePage title="Tresorerie reelle vs previsionnelle" subtitle="Solde bancaire, solde prevu, ecart et projection recalibree." rows={rows} columns={[
    ["month", "Mois"],
...
export function RunwayPage({ scenarioId, horizon }: V3Context) {
  ...
  return (
    <section className="space-y-5">
      <PageTitle title="Cash runway" subtitle="Runway base sur cash bancaire reel, burn et cash-in pondere." />
      ...
      <SimpleTable rows={(data?.recommendedActions ?? []).map((action: string, id: number) => ({ id, action }))} columns={[["action", "Action recommandee"]]} />
    </section>
  );
}
...
export function ForecastReliabilityPage() {
  ...
  return <TablePage title="Fiabilite previsionnelle" subtitle="Score par mois et facteurs de penalisation." rows={rows} columns={[
...
export function ClientPaymentProfilesPage() {
  ...
  return <TablePage title="Paiements clients" subtitle="Delais reels, retards et fiabilite de paiement." rows={rows} columns={[
    ["clientId", "Client"],
    ...
    ["reliabilityScore", "Fiabilite"]
  ]} />;
}

export function FinancialAnomaliesPage() {
  const { rows } = useRows("/financial-anomalies");
  return <TablePage title="Anomalies financieres" subtitle="Transactions inhabituelles, doublons, retards et ecarts." rows={rows} columns={[
    ["severity", "Severite", severityBadge],
    ["type", "Type"],
...
export function DataQualityPage() {
  ...
  return (
    <section className="space-y-5">
      <PageTitle title="Sante des donnees" subtitle="Qualite des donnees banque, compta, factures et rapprochements." />
      <KpiCard label="Score qualite" value={`${data?.score ?? 0}/100`} tone={(data?.score ?? 0) < 70 ? "risk" : "good"} />
      <SimpleTable rows={data?.issues ?? []} columns={[["severity", "Severite", severityBadge], ["type", "Type"], ["message", "Message"], ["suggestedFix", "Correction"], ["status", "Statut"]]} />
    </section>
  );
}
...
export function ConnectorSupervisionPage() {
  return <CrudPage title="Supervision connecteurs" path="/connectors" initial={{ organizationId: "", companyId: "", type: "banking", provider: "mock_bank_provider", name: "", status: "inactive", configuration: {} }} fields={[
    { name: "organizationId", label: "Organisation ID" }, { name: "companyId", label: "Societe ID" }, { name: "type", label: "Type" }, { name: "provider", label: "Provider" }, { name: "name", label: "Nom" }, { name: "status", label: "Statut" }, { name: "errorMessage", label: "Erreur" }
...
export function BankConsentsPage() {
  ...
  return <TablePage title="Consentements bancaires" subtitle="Gouvernance des consentements de lecture bancaire, sans stockage d'identifiants." rows={rows} columns={[
    ["provider", "Provider"], ["status", "Statut"], ["consentExpiresAt", "Expiration"], ["lastSyncAt", "Dernier sync"], ["createdBy", "Cree par"]
  ]} />;
}
...
export function CodirReportPage({ scenarioId, horizon }: V3Context) {
  ...
  return (
    <section className="space-y-5">
      <PageTitle title="Rapport CODIR connecte" subtitle="Rapport mensuel base sur reel bancaire, ecarts, anomalies et prevision recalibree." />
...
export function FinancialAuditPage() {
  ...
  return <TablePage title="Audit financier" subtitle="Actions sensibles liees banque, compta, rapprochement, imports et exports." rows={rows} columns={[
    ["createdAt", "Date"], ["entityType", "Entite"], ["entityId", "ID"], ["action", "Action"]
  ]} />;
}

export function FinancialRulesPage() {
  return <CrudPage title="Regles de categorisation bancaire" path="/bank/categorization-rules" initial={{ organizationId: "", name: "", priority: 10, isActive: true, condition: { labelContains: "URSSAF" }, targetCategoryId: "", autoApply: "if_high_confidence" }} fields={[
    { name: "organizationId", label: "Organisation ID" }, { name: "name", label: "Nom" }, { name: "priority", label: "Priorite", type: "number" }, { name: "targetCategoryId", label: "Categorie cible" }, { name: "autoApply", label: "Auto apply" }, { name: "isActive", label: "Active", type: "checkbox" }
  ]} columns={[{ key: "priority", label: "Priorite" }, { key: "name", label: "Regle" }, { key: "targetCategoryId", label: "Categorie" }, { key: "autoApply", label: "Mode" }, { key: "isActive", label: "Active", render: (row: any) => row.isActive ? "Oui" : "Non" }]} />;
}
