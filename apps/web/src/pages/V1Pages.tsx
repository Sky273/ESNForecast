...
export function ScenariosPage({ scenarioId, horizon }: V1Context) {
  ...
  useEffect(() => { void api<any[]>("/scenarios").then(setRows); }, []);
  useEffect(() => {
...
export function ProfitabilityMissionsPage({ scenarioId, horizon }: V1Context) {
  ...
  return <ProfitabilityTable title="Rentabilité missions" rows={rows} columns={[
    ...
    ["profitabilityBadge", "Badge", (v: string) => <Badge tone={v === "negative" ? "risk" : v === "weak" ? "warn" : "good"}>{v}</Badge>]
  ]} />;
}
...
export function BillingPage() {
  return <CrudPage title="Facturation prévisionnelle" path="/invoice-forecasts" initial={{ missionId: "", scenarioId: "", invoiceDate: "2026-06-30", dueDate: "2026-07-30", expectedPaymentDate: "2026-07-30", amountHT: 10000, vatRate: 0.2, amountTTC: 12000, status: "planned", probability: 1 }} fields={[
    { name: "missionId", label: "Mission ID" }, { name: "scenarioId", label: "Scénario ID" }, { name: "invoiceDate", label: "Date facture", type: "date" }, { name: "dueDate", label: "Échéance", type: "date" }, { name: "expectedPaymentDate", label: "Encaissement prévu", type: "date" }, { name: "amountHT", label: "Montant HT", type: "number" }, { name: "vatRate", label: "TVA", type: "number" }, { name: "amountTTC", label: "Montant TTC", type: "number" }, { name: "status", label: "Statut" }, { name: "probability", label: "Probabilité", type: "number" }
...
export function CashInPage() {
  return <CrudPage title="Encaissements prévisionnels" path="/cash-in-forecasts" initial={{ scenarioId: "", sourceType: "manual", expectedDate: "2026-07-30", amount: 10000, probability: 1, weightedAmount: 10000, status: "planned" }} fields={[
    { name: "scenarioId", label: "Scénario ID" }, { name: "sourceType", label: "Source" }, { name: "sourceId", label: "Source ID" }, { name: "expectedDate", label: "Date prévue", type: "date" }, { name: "amount", label: "Montant", type: "number" }, { name: "probability", label: "Probabilité", type: "number" }, { name: "weightedAmount", label: "Montant pondéré", type: "number" }, { name: "status", label: "Statut" }
  ]} columns={[{ key: "expectedDate", label: "Date" }, { key: "sourceType", label: "Source" }, { key: "amount", label: "Montant", render: (r: any) => money(r.amount) }, { key: "weightedAmount", label: "Pondéré", render: (r: any) => money(r.weightedAmount) }, { key: "status", label: "Statut" }]} />;
}

export function CashOutPage() {
  return <CrudPage title="Décaissements prévisionnels" path="/cash-out-forecasts" initial={{ scenarioId: "", sourceType: "manual", expectedDate: "2026-07-30", amount: 10000, status: "planned" }} fields={[
    { name: "scenarioId", label: "Scénario ID" }, { name: "sourceType", label: "Source" }, { name: "sourceId", label: "Source ID" }, { name: "expectedDate", label: "Date prévue", type: "date" }, { name: "amount", label: "Montant", type: "number" }, { name: "status", label: "Statut" }
  ]} columns={[{ key: "expectedDate", label: "Date" }, { key: "sourceType", label: "Source" }, { key: "amount", label: "Montant", render: (r: any) => money(r.amount) }, { key: "status", label: "Statut" }]} />;
}
...
export function SimulationsPage() {
  return <CrudPage title="Simulations" path="/simulation-events" initial={{ scenarioId: "", type: "exceptional_cost", label: "", startDate: "2026-08-01", amount: 10000, percentage: 0, parameters: {}, isActive: true }} fields={[
    { name: "scenarioId", label: "Scénario ID" }, { name: "type", label: "Type" }, { name: "label", label: "Libellé" }, { name: "startDate", label: "Début", type: "date" }, { name: "endDate", label: "Fin", type: "date" }, { name: "amount", label: "Montant", type: "number" }, { name: "percentage", label: "Pourcentage", type: "number" }, { name: "relatedMissionId", label: "Mission ID" }, { name: "isActive", label: "Active", type: "checkbox" }, { name: "notes", label: "Notes", type: "textarea" }
  ]} columns={[{ key: "label", label: "Simulation" }, { key: "type", label: "Type" }, { key: "startDate", label: "Début" }, { key: "amount", label: "Montant", render: (r: any) => money(r.amount ?? 0) }, { key: "isActive", label: "Active", render: (r: any) => r.isActive ? "Oui" : "Non" }]} />;
}
