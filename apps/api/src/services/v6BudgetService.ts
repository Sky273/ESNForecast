...
export function calculateAnnualLanding({ fiscalYear, budgetLines, rollingLines, actuals }: { fiscalYear: number; budgetLines: any[]; rollingLines: any[]; actuals: any[] }) {
  ...
  return {
    ...
    mainDrivers: [
      { label: "CA realise a date", impact: round(actualRevenue) },
...
export function calculateRequiredPipeline({ targetRevenue, actualRevenue, signedRemainingRevenue, weightedPipelineRevenue, conversionRate }: { targetRevenue: number; actualRevenue: number; signedRemainingRevenue: number; weightedPipelineRevenue: number; conversionRate: number }) {
  ...
  return {
    ...
    recommendations: [
      ...
      "Prioriser les offres a marge superieure a 28 %"
    ]
  };
}

export function calculateBudgetStaffing({ fiscalYear, budgetRevenue }: { fiscalYear: number; budgetRevenue: number }) {
  return Array.from({ length: 12 }, (_, index) => {
    const month = index + 1;
    const monthlyRevenue = budgetRevenue / 12;
    const requiredBillableDays = monthlyRevenue / 780;
    const internalCapacityDays = month < 9 ? 150 : 166;
    const externalCapacityDays = month < 9 ? 46 : 54;
