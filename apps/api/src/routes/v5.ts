...
v5Router.get("/ready", async (_req, res, next) => {
  try {
...
v5Router.get("/system/status", async (_req, res, next) => {
  try {
...
v5Router.get("/observability/summary", async (_req, res, next) => {
  try {
...
v5Router.get("/observability/logs", async (req, res, next) => {
  try {
...
v5Router.get("/observability/errors", async (req, res, next) => {
  try {
...
v5Router.get("/observability/slow-requests", async (req, res, next) => {
  try {
...
v5Router.get("/jobs", async (req, res, next) => {
  try {
...
v5Router.get("/jobs/:id", async (req, res, next) => {
  try {
...
v5Router.post("/jobs/:id/retry", async (req, res, next) => {
  try {
...
v5Router.post("/jobs/:id/cancel", async (req, res, next) => {
  try {
...
v5Router.get("/backoffice/organizations", async (_req, res, next) => {
  try {
...
v5Router.get("/backoffice/organizations/:id", async (req, res, next) => {
  try {
...
v5Router.get("/backoffice/organizations/:id/diagnostics", async (req, res, next) => {
  try {
...
v5Router.post("/backoffice/organizations/:id/recalculate", async (req, res, next) => {
  try {
...
v5Router.post("/backoffice/organizations/:id/resync", async (req, res, next) => {
  try {
...
v5Router.post("/backups", async (req, res, next) => {
  try {
...
v5Router.get("/backups", async (req, res, next) => {
  try {
...
v5Router.get("/backups/:id/download", async (req, res, next) => {
  try {
    ...
    res.json({ metadata: backup, data: { note: "Export demo V5 sans secrets provider.", generatedAt: new Date().toISOString() } });
  } catch (error) {
    next(error);
  }
});

v5Router.post("/restores/dry-run", async (req, res, next) => {
  try {
    const orgId = req.body?.organizationId ?? await organizationId();
    res.status(201).json(await db.restoreRun.create({ data: { organizationId: orgId, sourceBackupId: req.body?.sourceBackupId, mode: "dry_run", status: "success", resultSummary: { valid: true, warnings: ["Les tokens provider sont exclus de la restauration par defaut."] }, createdBy: "admin", completedAt: new Date() } }));
  } catch (error) {
    next(error);
  }
});

v5Router.post("/restores", async (req, res, next) => {
  try {
    const orgId = req.body?.organizationId ?? await organizationId();
...
v5Router.post("/exports/full", async (req, res, next) => {
  try {
    const orgId = req.body?.organizationId ?? await organizationId();
    const company = await firstCompany();
...
v5Router.get("/exports/:id/download", async (req, res, next) => {
  try {
...
v5Router.get("/retention-policies", async (_req, res, next) => {
  try {
...
v5Router.put("/retention-policies/:id", async (req, res, next) => {
  try {
...
v5Router.post("/purge-runs", async (req, res, next) => {
  try {
    const orgId = req.body?.organizationId ?? await organizationId();
...
v5Router.get("/security/events", async (req, res, next) => {
  try {
...
v5Router.get("/security/login-attempts", async (req, res, next) => {
  try {
...
v5Router.get("/security/sensitive-access", async (req, res, next) => {
  try {
...
v5Router.get("/security/sessions", (_req, res) => res.json([{ id: "demo-session", user: "admin@esnforecast.local", status: "active", lastSeenAt: new Date().toISOString() }]));
v5Router.post("/security/sessions/:id/revoke", (req, res) => res.json({ id: req.params.id, status: "revoked", correlationId: req.correlationId }));

v5Router.get("/feature-flags", async (_req, res, next) => {
  try {
...
v5Router.post("/feature-flags", async (req, res, next) => {
  try {
    res.status(201).json(await db.featureFlag.create({ data: req.body }));
  } catch (error) {
    next(error);
  }
});

v5Router.put("/feature-flags/:id", async (req, res, next) => {
  try {
    res.json(await db.featureFlag.update({ where: { id: req.params.id }, data: req.body }));
  } catch (error) {
    next(error);
  }
});

v5Router.delete("/feature-flags/:id", async (req, res, next) => {
  try {
...
v5Router.get("/onboarding", async (req, res, next) => {
  try {
...
v5Router.put("/onboarding/steps/:stepKey", async (req, res, next) => {
  try {
    const orgId = req.body?.organizationId ?? await organizationId();
    const current = await db.onboardingState.findFirst({ where: { organizationId: orgId } });
    const steps = { ...(current?.steps ?? {}), [req.params.stepKey]: Boolean(req.body?.completed ?? true) };
    res.json(current
...
v5Router.post("/onboarding/reset", async (req, res, next) => {
  try {
    const orgId = req.body?.organizationId ?? await organizationId();
...
v5Router.get("/help/contextual", async (req, res, next) => {
  try {
